...
def templateconst(template,mul):
    path=[]
    ...
    return temp
#polygon to text
header,footer,verties = extract('C:\\Users\\ktuser\\Downloads\\KLAWorkshop\\Milestone6\\Source.txt')
headertemp,footertemp,template = extract('C:\\Users\\ktuser\\Downloads\\KLAWorkshop\\Milestone6\\POI.txt')
...
mul=[1,1.5,0.5]
for i in range(3):
    temp = temp + templateconst(template,mul[i])
...
out = "".join(output)
with open('milestone 6.txt', 'w') as f:
    f.write(out)
